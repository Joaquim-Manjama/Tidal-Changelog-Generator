...
export const updateEntry = async (entryId: string, category:string, description: string, displayOrder: number) => {
    const token = localStorage.getItem("token");

    try {
        const response = await fetch(`${API_URL}/projects/releases/entries/${entryId}`, {
            method: "PUT",
            headers: {
                "Authorization": `Bearer ${token}`,
                "Content-Type": "application/json"
        ...
        console.error("Error updating entry display order:", error);
        throw error;
    }
};
