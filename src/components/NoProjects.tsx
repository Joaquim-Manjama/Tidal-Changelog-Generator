...
const NoProjects = () => {
    ...
    const handleSubmit = async(e: React.SubmitEvent<HTMLFormElement>) => {
        ...
        const response = await createProject(name, slug, githubRepo);
    ...
    if (isFormActive) {
        return<div className="absolute top-0 left-0 w-full h-screen flex transition duration-200">
            <form action=""  onSubmit={(e) => handleSubmit(e)} className="m-auto bg-white/30 backdrop-blur-xl m-auto w-[500px] rounded-2xl border border-white/10 shadow-xl p-8 flex flex-col gap-5">
                ...
                <button type="submit" className="bg-dark-teal-700 p-4 rounded-lg shadow-xl text-white hover:cursor-pointer hover:bg-dark-teal-800">+ Create</button>
            </form>
        </div>  
    }
